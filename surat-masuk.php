<?php 
// WAJIB: Menjalankan session agar bisa membaca role 'pimpinan'
session_start(); 
include "koneksi.php"; 

// Proteksi: Jika belum login, tendang balik ke login.php
...
<head>
    ...
    <title>Surat Masuk - SISUM TU</title>
        ...
        <aside class="w-64 bg-emerald-800 text-white hidden md:block shadow-xl">
            ...
            <nav class="mt-6 px-4">
                ...
                <a href="surat-masuk.php" class="flex items-center p-3 bg-emerald-700 rounded-lg mb-2 shadow-inner">
                    <i class="fas fa-envelope-open-text mr-3 text-emerald-300"></i> Surat Masuk
                </a>
        ...
        <main class="flex-1">
            <header class="bg-white shadow-sm p-4 flex justify-between items-center border-b border-emerald-100">
                <div class="flex items-center">
                    <h2 class="text-xl font-bold text-emerald-800 italic">Daftar Surat Masuk</h2>
                </div>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
                ...
                <?php if ($_SESSION['role'] !== 'staff') : ?>
                    <a href="tambah-surat-masuk.php" class="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-lg shadow-md transition flex items-center">
                        <i class="fas fa-plus mr-2 text-xs"></i> Registrasi Surat Baru
                    </a>
                <?php else: ?>
                    <div></div> <?php endif; ?>
                ...
                <div class="bg-white rounded-xl shadow-md border border-emerald-100 overflow-hidden">
                    <table class="w-full text-left">
                        <thead class="bg-emerald-800 text-white text-xs uppercase tracking-wider">
                            <tr>
                                <th class="p-4 font-semibold">Tgl Diterima</th>
                                <th class="p-4 font-semibold">No. Surat</th>
                                <th class="p-4 font-semibold">Asal Surat</th>
                        ...
                        <tbody class="divide-y divide-emerald-50">
                            <?php
                                $tampil = $koneksi->query("SELECT * FROM surat_masuk ORDER BY id_surat DESC");
                                while($data = $tampil->fetch_assoc()) :
                            ?>
                            <tr class="hover:bg-emerald-50/50 transition text-sm">
                                <td class="p-4"><?php echo date('d M Y', strtotime($data['tgl_terima'])); ?></td>
                                <td class="p-4 font-mono font-bold text-emerald-700"><?php echo $data['no_surat']; ?></td>
                                <td class="p-4"><?php echo $data['asal_surat']; ?></td>
                                <td class="p-4"><?php echo $data['perihal']; ?></td>
                                </td>
                                <td class="p-4">
                                    <div class="flex justify-center space-x-2">
                                        <?php if($_SESSION['role'] == 'pimpinan') : ?>
                                            <a href="edit-surat-masuk.php?id=<?php echo $data['id_surat']; ?>" 
                                            ...
                                            <a href="hapus-surat-masuk.php?id=<?php echo $data['id_surat']; ?>" 
                                        ...
                                        <a href="detail-surat-masuk.php?id=<?php echo $data['id_surat']; ?>" 
